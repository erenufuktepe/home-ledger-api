...
class Payment(BaseModel):
    ...
    amount: Decimal = Field(..., description="Monthly amount paid.", example=125.5)
...
class User(BaseModel):
    ...
    total_income: Decimal = Field(
        ..., description="Monthly total income for the person.", example=6200
    )
    total_expenses: Decimal = Field(
        ..., description="Monthly total expenses for the person.", example=3100
    )
    total_credit_card_balance: Decimal = Field(
        ..., description="Total credit card balances.", example=1800
    )
